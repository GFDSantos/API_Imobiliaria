...
def del_apartamento(query: ApartamentoBuscaSchema):
    ...
    if not apartamento:
        # se o apartamento não foi encontrado
...
def update_apartamento(form: ApartamentoSchema):
    """
    Atualiza os dados de um apartamento a partir do nome do condomínio
    """
    session = Session()
    apartamento = session.query(Apartamento).filter(
        Apartamento.condominio == form.condominio
    ).first()

    if not apartamento:
        return {"message": "Apartamento não encontrado para atualização :/"}, 404
    ...
    try:
        ...
        session.rollback()
        return {"message": f"Erro ao atualizar o apartamento: {e}"}, 400
